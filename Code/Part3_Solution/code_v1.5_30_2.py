    ...
    def soulution(self, SA_newV, SA_juge, juge_text, ValueRange, func):
        ...
        while Ti > self.temFinal:
            ...
            for i in range(self.Markov_chain):
                ...
                if boolV == True:
                    res_temp.append(
                        SA_juge(new=[newV[k][i] for k in range(ndim)], func=func, old=[preV[k][i] for k in range(ndim)],
                                T=Ti)[-1])
                    val_temp.append(
                        SA_juge(new=[newV[k][i] for k in range(ndim)], func=func, old=[preV[k][i] for k in range(ndim)],
                                T=Ti)[0])
...
# 导入k=200时的情况
centroid_ins = pd.read_csv(path + 'Centroid_30_2.csv')
now_ins = pd.read_csv(path + 'now_stadium.csv')
# 导入现有设施的数据
...
for i in range(row_centroid_ins):
    for j in range(row_now_ins):
        # 取出距离当前需求点最近的三个现有点
        distance = np.sqrt(np.square(centroid_ins.iloc[i, 0] - now_ins.iloc[j, 0]) + np.square(
            centroid_ins.iloc[i, 1] - now_ins.iloc[j, 1]))
    ...
    def func2(w):
        x, y = w
        fxy = 0.15 * np.sqrt(
            np.square(x - distance_cache.iloc[0, 0]) + np.square(y - distance_cache.iloc[0, 1])) + 0.15 * np.sqrt(
            np.square(x - distance_cache.iloc[1, 0]) + np.square(y - distance_cache.iloc[1, 1])) + 0.15 * np.sqrt(
            np.square(x - distance_cache.iloc[2, 0]) + np.square(y - distance_cache.iloc[2, 1])) - 0.53 * np.sqrt(
