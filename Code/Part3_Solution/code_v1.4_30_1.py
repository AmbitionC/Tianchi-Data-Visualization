...
# 导入k=200时的情况
centroid_ins = pd.read_csv(path + 'Centroid_30_1.csv')
now_ins = pd.read_csv(path + 'now_park.csv')
# 导入现有设施的数据
...
for i in range(row_centroid_ins):
    ...
    def func2(w):
        ...
        fxy = 0.1 * np.sqrt(
            ...
            np.square(x - distance_cache.iloc[2, 0]) + np.square(y - distance_cache.iloc[2, 1])) - 0.7 * np.sqrt(
